...
char **construction_tableau_chaine(char *line)
{
    ...
    char **elements = malloc(buffersize * sizeof(char*)); //Allocation memoire
    char *element;
...
int executer_ligne(char **args, char **envp)
{
    ...
    if (!strcmp(args[0], "cd")){
        ...
        if (pid == 0) {
            ...
            } while (!WIFEXITED(status) && !WIFSIGNALED(status)); //Renvoie true si fils terminee && termine a cause signal
        }
    }
    return 1;
...
int main(int argc,char *argv[], char *envp[]) {
    ...
    //1. Lire la ligne en entree
    //2. Transformer ligne en tableau de chaine grace a strtok()
    //3. Executer la commande (fork() puis execve())
    char *chaine;
    char **args;
    int execute;
